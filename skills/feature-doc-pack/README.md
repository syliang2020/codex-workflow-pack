...
- `ISSUES.md`
- `RELEASE.md`
- 可选 `PROGRESS.md`，仅在多人长期协作、任务板过大或用户明确要求单独进度视图时生成
...
- 任务归属、依赖、阻塞、接口进度摘要和测试进度摘要写入 `TASK-BOARD.md`。
- 接口开发状态和联调状态写入 `API-CONTRACT.md`。
- 规则验收、边界场景、联调检查和测试执行状态写入 `TEST-CASES.md`。
- 业务规则、接口、任务和测试用例之间应通过可点击编号建立追踪关系。

## 进度跟踪策略

默认不新增独立进度文档。feature-doc-pack 使用三层进度模型：

1. `TASK-BOARD.md`：任务级进度总览。
2. `API-CONTRACT.md`：接口开发和联调进度。
3. `TEST-CASES.md`：测试执行进度。

只有当多人长期协作、任务板过大、跨多个迭代周期，或用户明确要求单独看进度时，才生成 `PROGRESS.md`。不要默认生成 `TASK-PROGRESS.md`。

推荐开发中更新规则：

- 开始任务时，更新 `TASK-BOARD.md` 的任务状态。
- 完成接口实现时，更新 `API-CONTRACT.md` 的开发状态。
- 完成前后端联调时，更新 `API-CONTRACT.md` 的联调状态。
- 执行测试后，更新 `TEST-CASES.md` 的执行状态。
- 如果状态不一致，优先修正文档，不要让任务显示已完成但接口或测试仍未开始。
