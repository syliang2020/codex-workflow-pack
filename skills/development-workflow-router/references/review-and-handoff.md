...
1. `git status`
2. `git diff`
3. 是否符合 workflow 阶段要求。
4. 是否符合 `writing-plans`。
5. 是否有计划外修改。
6. 是否存在多个 subagent 修改同一文件或同一模块。
7. 构建结果。
8. 测试结果。
9. 是否存在前后端接口不一致。
10. 是否存在未处理 TODO、临时 mock 或占位实现。
...
- 后端复杂功能或复杂修复必须使用 `backend-feature-design-review` skill；如果 skill 不可用，说明原因并由主 agent 按同等标准检查。
- 后端功能在编码前已经使用过 `backend-feature-design-review` 时，编码后仍需再次审查。
